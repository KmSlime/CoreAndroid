...
namespace Buoi6_18DTHA1_DemoAPI.Controllers.api
{
    ...
    public class AuthenticateController : ControllerBase
    {
        ...
        public async Task<ActionResult> Login(string userName, string passWord)
        {
            try
            {
                ...
                if (result) // if user and password valid
                {
                    string securityKey = configuration["JWT:Secret"];
                    var symmetricSecurityKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(securityKey));
                    var userRoles = await userManager.GetRolesAsync(user);
                    ...
                    string strToken = new JwtSecurityTokenHandler().WriteToken(token);
                    UserLoginSuccessModel dataUser = new UserLoginSuccessModel()
                    {
                        UserName = user.UserName,
                        Token = strToken
                    };
                    return Ok(new { status = true, message = "Login success", data = dataUser });
                }
                return Ok(new { status = false, message = "Can not authenticate" });
            }
            catch (Exception ex)
